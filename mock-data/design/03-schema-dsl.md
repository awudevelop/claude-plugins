...
The Schema DSL (Domain Specific Language) is the foundation of MockData. It provides a declarative way to define data entities, their fields, relationships, and computed properties.
...
```typescript
import { defineData, field, hasOne, hasMany, belongsTo } from '@mockdata/schema';
...
```typescript
import { defineView, embed, pick } from '@mockdata/schema';
...
```typescript
import { defineEndpoint } from '@mockdata/schema';
...
```typescript
// schemas/index.ts
import { defineData, defineView, field, hasOne, hasMany, belongsTo, embed, pick } from '@mockdata/schema';
