  ...
  return {
    ...
    visitor: {
      ...
      ImportDeclaration(path: NodePath<t.ImportDeclaration>) {
        ...
        // Transform @mockdata imports
        if (source === '@mockdata/schema') {
          path.remove();
          return;
        }
      },
...
```typescript
import { defineData, field, hasMany } from '@mockdata/schema';
...
export function mockdataVitePlugin(): Plugin {
  return {
    name: 'mockdata',
    ...
    transform(code, id) {
      ...
      // Skip node_modules (except @mockdata)
      if (id.includes('node_modules') && !id.includes('@mockdata')) {
        return null;
      }
  ...
  apply(compiler: Compiler) {
    compiler.options.module.rules.push({
      ...
      use: {
        ...
        options: {
          plugins: [
            ['@mockdata/build/babel-plugin', { mode: process.env.NODE_ENV }],
          ],
        },
      },
    });
...
import { mockdataVitePlugin } from '@mockdata/build/vite-plugin';
...
```typescript
// webpack.config.js
const { MockdataWebpackPlugin } = require('@mockdata/build/webpack-plugin');
    ...
    if (!isServer && process.env.NODE_ENV === 'production') {
      config.module.rules.push({
        ...
        use: {
          ...
          options: {
            plugins: [
              ['@mockdata/build/babel-plugin', { mode: 'production' }],
            ],
          },
        },
      });
